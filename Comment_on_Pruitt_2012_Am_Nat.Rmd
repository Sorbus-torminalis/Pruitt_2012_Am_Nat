...
This comment reports an attempt to detect duplications in @Pruitt2012.
...
The number of snails per mesocosm is varied in replicates A and B, but mostly consistent in replicate C (Fig. \@ref(fig:n-snail-plot)).
Within each mesocosm the data are sorted by survival and partially sorted by snail size (Fig. \@ref(fig:raw-data-plot)).

```{r n-snail-plot, fig.cap = "Number of snails per mesocosm in the three replicates apparently measured to one (dark purple) or two (light green) decimal places.", fig.height = 3, fig.width = 6}
raw_data %>% 
  mutate(
...
```{r raw-data-plot, fig.cap = "Size of snails that died (blue) or survived (orange) against row number from three mesocosm.", fig.height = 3.5, fig.width = 4}
#example of raw data
raw_data %>% 
  group_by(mesocosm) %>% 
  mutate(row = 1:n()) %>%
...
rugg_rand_hist <- rugg_rand_max %>% 
  ...
  labs(x = "Maximum Ruggiero similarity in permutation")
...
(rugg_size_hist + rugg_rand_hist) / rugg_raster +
  patchwork::plot_annotation(tag_levels = "A") + plot_layout(heights = c(0.2, 0.8))
```
